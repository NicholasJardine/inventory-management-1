...
const Sidebar = () => {
  ...
  const sidebarClassNames = `fixed flex flex-col ${
    isSidebarCollapsed ? "w-0 md:w-16" : "w72 md:w-64"
  ...
  return (
    <div className={sidebarClassNames}>
        ...
        <Image src="https://nj-inventory-management-1.s3.af-south-1.amazonaws.com/logo.png"
                        alt="my-stock-logo"
                        width={27}
                        height={27}
                        className="rounded-full w-8" />            <h1 className={`font-extrabold text-2xl ${isSidebarCollapsed ? "hidden" : "block" }`}>MyStock</h1>
...
<SidebarLink href="/expenses" icon={CircleDollarSign} label='Expenses' isCollapsed={isSidebarCollapsed}/>
    ...
    <div className={`${isSidebarCollapsed ? "hidden" : "block"} mb-10`}>
        <p className='text-center text-xs text-gray-500'>&copy; 2024 MyStock</p>
    </div>
    </div>
